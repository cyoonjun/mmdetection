...
class YTVOSPropDataset(CustomDataset):
    ...
    def get_clip_ids(self, idx, T):
        # get clip frames' idx in the video
        vid, frame_id = idx
        vid_info = self.vid_infos[vid]
        vid_len = len(vid_info['filenames'])
        
        min_index = max(0, frame_id - T)
        max_index = min(vid_len - 1, frame_id + T)

        frame_list = range(min_index, max_index + 1)
        return frame_list
    ...
    def prepare_test_img(self, idx):
        """Prepare a video clip for testing (multi-scale and flipping)"""
        vid, frame_id = idx
        vid_info = self.vid_infos[vid]
        img_info = vid_info
        clip_frame_ids = self.get_clip_ids(idx, 6)
        ...
        for frame_id in clip_frame_ids:
            result = dict(
                ...
                ref_frame_id=None)
            if self.proposals is not None:
                result['proposals'] = self.proposals[idx]
            self.pre_pipeline(result)
            img = self.pipeline(result)['img'][0]
            img_metas = self.pipeline(result)['img_metas'][0].data
            clip_imgs.append(img)
            clip_filenames.append(img_metas['filename'])
            clip_ori_filenames.append(img_metas['ori_filename'])

        results['clip_imgs'] = clip_imgs
        results['clip_filenames'] = clip_filenames
        results['clip_ori_filenames'] = clip_ori_filenames
        results['clip_frame_ids'] = list(clip_frame_ids)
